...
def get_channel_info(folder_path: FolderPathType):
    ...
    for processor in processors:
        if processor.get("@name", None) != "Sources/Rhythm FPGA":  # TODO: not sure how reliable this is
            continue
        index_to_name = {}
        ...
        break
    assert len(index_to_name) == len(list(folder_path.glob("*.continuous")))
    ...
    if len(index_to_name) == 32:  # 32-channel V-probe
        channel_maps = DEFAULT_CHANMAP
        probe_maps = np.zeros(len(channel_maps))
    elif len(index_to_name) == 40:  # 16-channel V-probe
        channel_maps = np.arange(9, 25, 1)
        probe_maps = np.zeros(len(channel_maps))
    elif len(index_to_name) == 64:  # 64-channel V-probe
        channel_maps = np.concatenate([DEFAULT_CHANMAP + 32, DEFAULT_CHANMAP])
        probe_maps = np.zeros(len(channel_maps))
    elif len(index_to_name) == 72:  # 64-channel V-probe
        channel_maps = np.concatenate([DEFAULT_CHANMAP + 32, DEFAULT_CHANMAP])
        probe_maps = np.zeros(len(channel_maps))
    elif len(index_to_name) == 144:  # 64-channel V-probe, two runs
        channel_maps = np.concatenate([DEFAULT_CHANMAP + 32, DEFAULT_CHANMAP])
        probe_maps = np.zeros(len(channel_maps))
    elif len(index_to_name) == 136:  # Two 64-channel V-probes
    ...
    for key in channel_map_dict.keys():
        assert key in index_to_name.values(), f"{key}"

    channel_map_dict = {
        key: channel_map_dict.get(index_to_name[key], index_to_name[key]) for key in index_to_name.keys()
    }
    return channel_map_dict, probe_map_dict, settings
    ...
    for data_dir in data_dirs:
        ...
        for data_file in file_list:
            if data_file.suffix == ".continuous":
                ...
                assert ch_name in channel_mapping.keys()
                ch_name = channel_mapping[ch_name]
...
def make_temporary_openephys_binary_tree(folder_path: FolderPathType, dat_folder_paths: list[FolderPathType]):
    ...
    channel_map_dict, probe_map_dict, settings = get_channel_info(data_dir)
    ...
    for processor in processors:
        if processor.get("@name", None) == "Sources/Rhythm FPGA":  # TODO: not sure how reliable this is
            ...
            break
    assert source_processor is not None, "No source processor found"
    source_processor_name = source_processor.get("@name").split("/")[1]
    source_processor_id = int(source_processor.get("@NodeId"))
    sample_rate_string = source_processor["EDITOR"].get("@SampleRateString")
    sample_rate = float(sample_rate_string.split(" ")[0])
    if "kS/s" in sample_rate_string:
        sample_rate *= 1000.0
    channel_info = source_processor["CHANNEL_INFO"]["CHANNEL"]
        ...
        continuous_dict["channels"].append(
            {
                ...
                "bit_volts": gain,
                "units": "uV",
                "source_processor_index": remapped_index - 1,
