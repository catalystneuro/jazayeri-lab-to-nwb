...
def get_channel_info(folder_path: FolderPathType):
    ...
    if len(num_to_name) == 32:  # 32-channel V-probe
        ...
        probe_maps = np.zeros(len(channel_maps), dtype=int)
        ...
        probe_maps = np.array([0, 1], dtype=int).repeat(64)
    else:
...
def make_temporary_openephys_binary_tree(folder_path: FolderPathType, dat_folder_paths: list[FolderPathType]):
    ...
    with open(data_dir / "settings.xml", "r") as f:
        settings_xml_text = f.read()
    if "OPENEPHYS" in settings_xml_text:
        with open(temp_settings_path, "w") as f:
            f.write(settings_xml_text.replace("OPENEPHYS", "RAWBINARY"))
    else:
